						...
						$msg	=	'<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
									...
									<body style="margin:0px; padding:0px; font: 14px/18px "Source Sans Pro"; background:#F3F2E9; color:#666;">
										...
										<div id="e_wrapper" style="display:block; margin:0px auto; padding:0; width:600px;">		
											...
											<div class="clear" style="margin:0; padding:0; height:0; clear:both;"></div>
											<!-- Start Content -->
											<div id="e_content" style="display:block; margin:0px; padding:0;">'.nl2br($message).'<br /><a target="_blank" href="http://www.timeanddate.com/worldclock/fixedtime.html?msg='.str_replace(" ","+",get_the_title($id)).'&amp;iso='.$hangout_mail_date.'T'.$output->hangout_time .'"> </a>
											</div>
											<!-- End Content -->
											<div class="clear" style="margin:0; padding:0; height:0; clear:both;"></div>
...
echo $curtime = mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('Y'));
	echo "<br>";
echo 	$curtime5 = mktime(date('H')+1,date('i'),date('s'),date('m'),date('d'),date('Y'));
	
	$webinar_id_reminders	=	$wpdb->get_results("select * from `".$subscriber_table."` where `reminder_time` BETWEEN '$curtime' and '$curtime5' ");
	// echo "<pre>";
	// print_r($webinar_id_reminders);
	// echo "<pre>";
	foreach($webinar_id_reminders as $post_id){
	...
	if(count($minutes_posts)>0) {
		while (have_posts()) : the_post();
			...
			// $tym_zon	=	get_post_meta(get_the_ID(),"hangout_time_zone",true);
			// $tz = get_city_id($tym_zon);
			$result=$wpdb->get_results('SELECT distinct(email), name,add_email,hangout_time,hangout_date from '.$subscriber_table.' where g_event_id="'.$id.'"');
			...
			foreach($result as $output){
				...
				$subject = str_replace("{eventlinkURL}",$eventlinkURL,$subject);
				$subject = str_replace("{eventName}",$eventName,$subject);
				$subject = str_replace("{time}",' '.$output->hangout_time ,$subject);
				$message = str_replace("{eventName}",$eventName,$message);
				$message = str_replace("{eventlinkURL}",$eventlinkURL,$message);
				$message = str_replace("{creatorEmail}",$creatorEmail,$message);
				$message = str_replace("{time}",' '.$output->hangout_time ,$message);
				$subject = str_replace("{name}",$output->name,$subject);
				$message = str_replace("{name}",$output->name,$message);
...
echo $curtime = mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('Y'));
	echo "<br>";
echo 	$curtime5 = mktime(date('H'),date('i')+5,date('s'),date('m'),date('d'),date('Y'));
