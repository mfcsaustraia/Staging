...
$timeapizone = $timeapidata->timezone->offset;
		...
		$newtimezone = $_POST["hangout_timezone"] .' ' .$timeapizone;
...
update_post_meta($post_id,"hangout_title",$_POST["hangout_record_embed_code_url"]);
	...
	{
		if($key != 'hangout_id' || $key != 'hangout_title' || $key != 'new_hangout_submit')
		{
...
if(isset($_REQUEST['EID'])){
	...
	$timezone = $timedata[0]." ".$timedata[1]." ".$timedata[2];
...
<div id="myMenu1" class="gh_accordian_tab"><i class="icon-plus-sign"></i> Event Details  </div>
                        ...
                        <div class="row-fluid">
							...
							<?php $locatation = $wpdb->get_results('select * from '. $wpdb->prefix.'location'); 
				...
				for($i = 1; $i <= 24; $i++){
					...
					?>
						...
						<div <?php if($hangout_registration_system=='default' || $hangout_registration_system==''){  echo 'style="display:none;"'; } ?> id="hangout_autoresponder">
								...
								<div id="Mailchimp" <?php if($hangout_registration_system!='Mailchimp'){  echo 'style="display:none;"'; } ?>>
									...
									<div id="Other" <?php if($hangout_registration_system!='Other'){  echo 'style="display:none;"'; } ?>>
										<div class="row-fluid-outer">
										<div class="row-fluid">
					...
					<?php /*
					<div id="myMenu20" class="gh_accordian_tab"><i class="icon-plus-sign"></i> 2 X Membership </div>
						<div id="myDiv20" class="gh_accordian_div">
						   <div class="row-fluid-outer">
								<div class="row-fluid">
									<div class="span4">
										<strong>Enable 2x Membership </strong>
									</div>
									<div class="span8">
										<div class="row-fluid">
											<input type="radio" name="ghanghout_enabled_membership"  class="ghanghout_enabled_membership" value="no" <?php if($ghanghout_enabled_membership=='' || $ghanghout_enabled_membership== "no")echo 'checked="checked"';?>> No &nbsp;&nbsp;
											<input type="radio" class="ghanghout_enabled_membership" name="ghanghout_enabled_membership"  value="yes" <?php if($ghanghout_enabled_membership == "yes")echo 'checked="checked"';?>> Yes
										</div>
									</div>
								</div>
							</div>
							<div class="is_mebership_active" <?php if($ghanghout_enabled_membership == "no") echo 'style="display:none;"'; ?>>
							<?php if (is_plugin_active('InstaMember/instamember.php') || is_plugin_active('wishlist-member/wpm.php') || is_plugin_active('memberrocket/functions.php')){ ?> 
							<div class="row-fluid-outer">
								<div class="row-fluid">
									<div class="span4">
									...
									<div class="span8">
										<div class="row-fluid">
											<select name="ghangout_membership_Type" class="ghangout_membership_type" />
											 <?php 
													$first='';
													$sel='';
													if (is_plugin_active('InstaMember/instamember.php')) {
														$first='instamember';
														if($ghangout_membership_Type=='instamember'){ $sel='selected'; }
														echo '<option value="instamember" '. $sel .' >Insta Member </option>';
													}	
													if (is_plugin_active('wishlist-member/wpm.php')) {
														$sel='';
														if($ghangout_membership_Type=='wpm'){ $sel='selected'; }
														if($first==''){ $first='wpm'; }
														echo '<option value="wpm" '. $sel .'>Wishlist Plugin </option>';
													}
													if (is_plugin_active('memberrocket/functions.php')) {
														$sel='';
														if($ghangout_membership_Type=='memberrocket'){ $sel='selected'; }
														if($first==''){ $first='memberrocket'; }
														echo '<option value="memberrocket" '. $sel .'>Member Rocket </option>';
													}

													if($ghangout_membership_Type!=''){
														$first=$ghangout_membership_Type;
													}
											?>
											</select>
										</div>
							...
							<div class="row-fluid-outer">
								<div class="row-fluid">
									<div class="span4">
										<strong>Member Access Live Page </strong>
									</div>
									<div class="span8">
										<div class="row-fluid">
											<?php 
										
									$instaprd = $wpdb->get_results('select id,title from '. $wpdb->prefix.'stms_products');
									$str='';
									echo '<div class="instamember 2xmembership"';
									if($first!='instamember') echo 'style="display:none;"';
									echo '>';

									foreach($instaprd as $members){
										$sel='';
										if(in_array($members->id,$ghangout_live_page_member_access))$sel="checked";
										echo '<input class="memberchk" type="checkbox" name="ghangout_live_page_member_access[]" value="'.$members->id.'" '.$sel.'> &nbsp;'.$members->title.' &nbsp;&nbsp;';
									}
									echo '</div>';

									$wish_option_value = $wpdb->get_var('select option_value from '. $wpdb->prefix.'wlm_options where option_name="wpm_levels"');
									$wish_option_data = unserialize($wish_option_value);
									
									echo '<div class="wpm 2xmembership"';
									if($first!='wpm') echo 'style="display:none;"';
									echo '>';
									foreach($wish_option_data as $id=>$data){
										$sel='';
										if(in_array($id,$ghangout_live_page_member_access))$sel="checked";
										echo '<input class="memberchk" type="checkbox" name="ghangout_live_page_member_access[]" value="'.$id.'" '.$sel.'> &nbsp;'.$data['name'].' &nbsp;&nbsp;';
									}
									echo '</div>';


									$rocketmember = $wpdb->get_results('select name,prod_code from '. $wpdb->prefix.'msys_products');
									
									echo '<div class="memberrocket 2xmembership"';
									if($first!='memberrocket') echo 'style="display:none;"';
									echo '>';
									foreach($rocketmember as $member){
										$sel='';
										if(in_array($member->prod_code,$ghangout_live_page_member_access))$sel="checked";
										echo '<input class="memberchk" type="checkbox" name="ghangout_live_page_member_access[]" value="'.$member->prod_code.'" '.$sel.'> &nbsp;'.$member->name.' &nbsp;&nbsp;';
									}
									echo '</div>';
									?>
							...
							<div class="row-fluid-outer">
								<div class="row-fluid">
									<div class="span4">
										<strong>Member Access Event Page </strong>
									</div>
									<div class="span8">
										<div class="row-fluid">
										<?php 
											...
											foreach($instaprd as $members){
												$sel='';
												if(in_array($members->id,$ghangout_event_page_member_access))$sel="checked";
												echo '<input class="memberchk" type="checkbox"  '. $sel .' name="ghangout_event_page_member_access[]" value="'.$members->id.'"> &nbsp;'.$members->title.' &nbsp;&nbsp;';
											} 
											echo '</div>';
											$str=''; 
											echo '<div class="wpm 2xmembership"';
											...
											foreach($wish_option_data as $id=>$data){
												$sel='';
												if(in_array($id,$ghangout_event_page_member_access))$sel="checked";
												echo '<input class="memberchk" type="checkbox"  '. $sel .' name="ghangout_event_page_member_access[]" value="'.$id.'"> &nbsp;'.$data['name'].' &nbsp;&nbsp;';
											}
											echo '</div>';
											...
											foreach($rocketmember as $member){
												$sel='';
												if(in_array($member->prod_code,$ghangout_event_page_member_access))$sel="checked";
												echo '<input class="memberchk" type="checkbox" name="ghangout_event_page_member_access[]" value="'.$member->prod_code.'" '.$sel.'> &nbsp;'.$member->name.' &nbsp;&nbsp;';
											}
											echo '</div>'; ?>
										</div>
									</div>
							...
							<div class="row-fluid-outer">
								<div class="row-fluid">
									<div class="span4">
										<strong>Member Access Re-Play Page </strong>
									</div>
									<div class="span8">
										<div class="row-fluid">
										<?php 
											...
											foreach($instaprd as $members){
												$sel='';
												if(in_array($members->id,$ghangout_replay_page_member_access))$sel="checked";
												echo '<input class="memberchk" type="checkbox"  '. $sel .' name="ghangout_replay_page_member_access[]" value="'.$members->id.'"> &nbsp;'.$members->title.' &nbsp;&nbsp;';
											} 
											echo '</div>';
											$str=''; 
											echo '<div class="wpm 2xmembership"';
											...
											foreach($wish_option_data as $id=>$data){
												$sel='';
												if(in_array($id,$ghangout_replay_page_member_access))$sel="checked";
												echo '<input class="memberchk" type="checkbox"  '. $sel .' name="ghangout_replay_page_member_access[]" value="'.$id.'"> &nbsp;'.$data['name'].' &nbsp;&nbsp;';
											}
											echo '</div>';
											...
											foreach($rocketmember as $member){
												$sel='';
												if(in_array($member->prod_code,$ghangout_replay_page_member_access))$sel="checked";
												echo '<input class="memberchk" type="checkbox" name="ghangout_replay_page_member_access[]" value="'.$member->prod_code.'" '.$sel.'> &nbsp;'.$member->name.' &nbsp;&nbsp;';
											}
											echo '</div>'; 
											?>
										</div>
							...
							<div class="row-fluid-outer">
								<div class="row-fluid">
									<div class="span4">
										<strong>Member Error Message </strong>
									</div>
									<div class="span8">
										<div class="row-fluid">
											<textarea name="ghangout_member_msg" class="longinput" id="ghangout_member_msg"><?php echo $ghangout_member_msg; ?></textarea>
										</div>
									</div>
								</div>
							</div>
							...
							<div class="row-fluid-outer">
								<div class="row-fluid">
									...
									<div class="span8">
										<div class="row-fluid">
											<input type="text" name="ghangout_member_login_url" class="longinput" value="<?php echo $ghangout_member_login_url; ?>" id="ghangout_member_login_url"/>
										</div>
									</div>
								</div>
							</div>
							<?php } else { ?>
								...
								<div class="row-fluid">
									<div class="span12"><strong>
										...
										To use 2x Membership Functionality you have to install Any of Below Plugins:-<br></strong>	
										1. Insta Member<br>
										2. Wishlist Plugin<br>
										3. Member Rocket<br></strong>
									</div>
								</div>
								</div>
							<?php } ?>
					...
					if(($google_hangout_theme != "" or $selected_theme!='') and ($selected_theme !='Default' and $google_hangout_theme != "Default" )){
						...
						<div id="option1layout" <?php if($g_hangout_layout_type==2){ echo 'style="display:none;"'; } ?>>
						 <div class="row-fluid-outer">
                        <div class="row-fluid">
                        ...
                        <div class="row-fluid">
                            ...
                            <div class="span8">
								...
								<?php if($ghanghout_logo!=''){ ?>
									<br><img width="100" src="<?php echo $baseurl.'/'.$ghanghout_logo;?>">
								<?php } ?>
