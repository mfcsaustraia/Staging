 ...
 * Description: Another <a href="http://www.universalmedia-online.com" target="_blank">UMO</a> plugin. Allowing you to create Webinars from hangouts with registrations and follow ups. <a href=" http://runclick.com/changelog" target="_blank" />Change Log </a>
  * Description: Another <a href="http://www.universalmedia-online.com" target="_blank">UMO</a> plugin. Allowing you to create Webinars from hangouts with registrations and follow ups. <a href=" http://runclick.com/changelog" target="_blank" />Change Log </a>
 ...
 class Umohangoutupdater
	{
		...
		public $excludefiles	=	array('google_hangout_plugin.php');
		...
		function live_popup_check(){
			...
			echo $live_buy_form_push.'-'.$live_vote_form_push.'-';
		...
		function checkupdates()
		{
			...
			} elseif(strtotime(get_option('hangout_check_license_date',true)) == strtotime(date('d-m-Y')) && get_option('umo_hangout_updated_message') != 1)
			{
			...
			else
			{
				...
				$result	=	array('update' => 0, 'version' => 0);
			...
			{
				update_option('umo_hangout_updated_message',0);
			}
			
			
			
			$isupdating		=	isset( $_GET['umo_hangout_update'] )?1:0;
			...
			if($result['update']=='1')
			{
				...
				add_action('admin_notices',array(&$this,'showupdatemessage') );
				
				include('google_hangout_plugin_connector.php');
				//create menu for update
			}
			else
			{
				include('google_hangout_plugin_connector.php');
		...
		{
			echo '<div id="server-seo-update-success-message" class="updated fade" style=" height: 44px !important; padding-top: 5px !important;"><p><strong>Updated successfully.</strong> Webinar Plugin.</p></div>';
		}
		
		function showupdatemessage()
		{
			echo '<div id="server-seo-update-message" class="error" style=" min-height: 44px !important; padding-top: 5px !important;"><p><strong>Please update</strong> Webinar Plugin. <span id="seo-server-update-progress" style="display:none;" >Updating <img src="http://www.tantrasxm.com/images/gallery-loader.gif"  /> Don\'t close window while update is in progress. </span> <a onclick="document.getElementById(\'seo-server-update-link\').style.display=\'none\';  document.getElementById(\'seo-server-update-progress\').style.display=\'block\';" id="seo-server-update-link" href="'.site_url().'/wp-admin/index.php?umo_hangout_update=1">click here to update</a></p></div>';
		}
		
		function showpermissionerror()
		{
			echo '<div id="server-seo-perm-message" class="error" style=" height: 44px !important; padding-top: 5px !important;"><p><strong>update failed </strong> Webinar Plugin folder is not writable. </p></div>';
		}
...
class G_Hangout {
	...
	function admin_init() {
		 ...
		 * Chat Operator role
		 *//*
		$op_role = get_role( 'g_hangout_op' );
		
		// Add capabilities to chat operators
		$op_role->add_cap( 'read' ); 
		$op_role->add_cap( 'edit_posts' ); 
		$op_role->add_cap( 'delete_posts' ); 
		$op_role->add_cap( 'moderate_comments' ); 
		$op_role->add_cap( 'list_users' ); 
	...
	function compatibility() {
        ...
        /*if( empty( $current_version ) or version_compare( $current_version, $this->version, '<') ) {
            ...
            update_option( 'g_hangout_plugin_version', $this->version );
...
// Take over the Plugin info screen
	//add_filter('plugins_api', 'g_hangout_plugin_api_call', 10, 3);
