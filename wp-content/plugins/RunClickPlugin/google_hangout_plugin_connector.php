...
function google_hangout_create_location(){
	global $wpdb;
		$chkcol = $wpdb->get_results("SELECT * FROM `".$wpdb->prefix."google_hangout_subscriber` limit 0,1");

	/* Custom fields for subscriber form */
	if(!isset($chkcol[0]->organization) && !isset($chkcol[0]->hangout_date) && !isset($chkcol[0]->reminder_time) && !isset($chkcol[0]->hangout_time)){
		$sql1_alter="ALTER TABLE `".$wpdb->prefix."google_hangout_subscriber`  ADD `organization` VARCHAR(255) NOT NULL , ADD `hangout_date` DATE NOT NULL , ADD `hangout_time` TIME NOT NULL , ADD `reminder_time` TIME NOT NULL";
...
/*
function movefile(){
			$templateDir = plugin_dir_path(__FILE__);
			$themeFolder = strstr(get_bloginfo('template_directory'),'wp-content/themes/');
				...
				@copy($templateDir . '/' . $templateFile1, ABSPATH . $themeFolder . '/' . $templateFile1);
				@copy($templateDir . '/' . $templateFile2, ABSPATH . $themeFolder . '/' . $templateFile2);
}
add_action( 'admin_head', 'movefile' );
*/
...
class GoogleHangoutPlugin 
 {
	...
	*/
	...
	{	 
			global $wpdb;
			update_option('umo_hangout_version','3.0.7');
	
			update_option('umo_hangout_licenceurl','http://runclick.com/');
			...
			update_option('g_hangout_reminder_subject', 'Your Webinar Begins {time} hours from now');
			...
			$themeFolder = strstr(get_bloginfo('template_directory'),'wp-content/themes/');
				$templateFile1 = 'single-ghangout.php';
				$templateFile2 = 'ghangout-style.php';
				if(!file_exists(ABSPATH.$themeFolder.'/'.$templateFile1)) @copy($templateDir . '/' . $templateFile1, ABSPATH . $themeFolder . '/' . $templateFile1);
				if(!file_exists(ABSPATH.$themeFolder.'/'.$templateFile2)) @copy($templateDir . '/' . $templateFile2, ABSPATH . $themeFolder . '/' . $templateFile2);
			*/	
	...
	jQuery(function($){
		$(document).ready(function(){
			//jQuery.noConflict();
				...
				/*$('#hangout_time_zone').change(function(){
					timezone = $(this).val();
					timeval = $('#ghangout_timezone').val();
					
					sendata = 'g_timezone='+timezone+'&timeval='+timeval;
						$("#rest_time").hide();
						$('#is_right_time').hide();
						//$('#ajax_loader').show();
						
					//alert(sendata);
					//alert('<?php echo plugin_dir_url(__FILE__);?>/ajax.php');
					$.ajax({
					type: 'POST',
					url: '<?php echo plugin_dir_url(__FILE__);?>ajax1.php',
					data: sendata,
					}).done(function(data) {

							alert(data);
						$('#ajax_loader').hide();
						$("#event_start_time").html(data);
						$("#rest_time").show();
						$('#is_right_time').show();
		...
		$('#get_youtube_details').click(function(){
				...
				url: '<?php echo plugin_dir_url(__FILE__);?>ajax_set_up_aweber.php',
				...
				/*$('#allow_access_event').click(function(){
					var app_id=$('#hangout_Aweber_app_id').val();
					var api_url="https://auth.aweber.com/1.0/oauth/authorize_app/"+app_id;
					window.open(api_url);
...
/*
	...
	function google_hangout_cron_mail(){
		global $wpdb;
		$wpdb->query('INSET INTO testing_cron VALUES("","inserting")');
		include('cron.php');
	}
