...
<input type="checkbox" style="display:inline; padding:0px; margin:0px 10px 15px 0px; height:auto;" name="makenow" id="watch_now" value="1" class="user-success"><span>I prefer to watch now</span> 
	...
	/*
							//start code for additional email in form
							<div class="hide_show">
	...
	$form .= '<div id="ghangout_event_form" style="clear:both;">
				<div class="box round">
					<form id="g_hangout_reg_form" method="post" name="hangout_reg_form">
						<div class="actionText">Register Now</div>
						<div>
							<input id="event_reg_name" name="event_reg_name" class="text" size="20" value="Enter Your Name" onblur="if (this.value == \'\') {this.value = \'Enter Your Name\';}" onfocus="if (this.value == \'Enter Your Name\') {this.value = \'\';}" type="text" taborder="2">
						</div>
						<div>
							<input id="event_reg_email" name="event_reg_email" class="text" size="20" value="Enter Your Email" onblur="if (this.value == \'\') {this.value = \'Enter Your Email\';}" onfocus="if (this.value == \'Enter Your Email\') {this.value = \'\';}" type="text" taborder="3">
						</div>
						<div>
							<input type="button" name="additonal_email"  id="additional_email"  value="Add additional email" />
						</div>
						<div id="additional_email_p" style="display:none;"> 
							<span>
								<input type="text" name="event_reg_email_add" id="event_reg_email_add" class="text" size="20" />
							</span>
						</div>
						<p>
							<input type="hidden" name="g_hangout_id" value="'.$post->ID.'" >
							<input name="submit" id="g_hangout_submit_form" type="image" border="0" src="'.plugin_dir_url(__FILE__).'images/GetEarlyAccessYellow.png" width="275" height="86">
						</p>
					</form>
					<p><p class=privacyText><img src="'. plugin_dir_url(__FILE__).'images/icon_padlock.png"> We respect your privacy 100%</p></p>	
				</div>
			</div>';*/
	} else {
		...
		elseif($hangout_registration_system == 'GetResponse')
		{
			$aweber_name	=	'name';
			$aweber_email	=	'email';
		}
		elseif($hangout_registration_system == 'InfusionSoft')
		{
			$aweber_name	=	'inf_field_FirstName';
			$aweber_email	=	'inf_field_Email';
		}
		elseif($hangout_registration_system == 'Sendreach')
		{
			$aweber_name	=	'name';
			$aweber_email	=	'email';
		}
		elseif($hangout_registration_system == 'ImnicaMail')
		{
			$aweber_name	=	'name';
			$aweber_email	=	'email';
		}
		elseif($hangout_registration_system == 'Mailchimp')
		{
			$aweber_name	=	'NAME';
			$aweber_email	=	'EMAIL';
		}
		elseif($hangout_registration_system == 'ArpReach')
		{
			$aweber_name	=	'first_name';
			$aweber_email	=	'email_address';
		}
		elseif($hangout_registration_system == 'Icontact')
		{
			$aweber_name	=	'fields_fname';
			$aweber_email	=	'fields_email';
		}
		elseif($hangout_registration_system == '1ShopingCart')
		{
			$aweber_name	=	'Name';
			$aweber_email	=	'Email1';
		}
		elseif($hangout_registration_system == 'Sendpeppe')
		{
			$aweber_name	=	'name';
			$aweber_email	=	'email';
		}
